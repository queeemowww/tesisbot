...
weight_builder = {}
volume_builder = {}
shipper_name_builder = {}
shipper_phone_builder = {}
consignee_name_builder = {}
consignee_phone_builder = {}
time_builder = {}
phone_builder = {}

@router.callback_query(F.data == "order", StateFilter(None))
async def order_1(callback: types.CallbackQuery, state: FSMContext):
    get_previous(callback.message)
    ...
    await callback.message.answer('1/10 \- *_Введите аэропорт/город отправления_*', reply_markup=departure_builder[callback.message.chat.id].as_markup(resize_keyboard=True))
...
async def order4(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.edit_text('<code>' + callback.message.text + '</code>', parse_mode=ParseMode.HTML, reply_markup=None)
    await callback.message.answer('2/10 \- *_Введите аэропорт/город прибытия_*', reply_markup=destination_builder[callback.message.chat.id].as_markup(resize_keyboard=True))
...
async def order7(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.edit_text('<code>' + callback.message.text + '</code>', parse_mode=ParseMode.HTML, reply_markup=None)
    await callback.message.answer('3/10 \- *_Введите количество мест_*:', reply_markup=pieces_builder[callback.message.chat.id].as_markup(resize_keyboard=True))
...
async def order10(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.edit_text('<code>' + callback.message.text + '</code>', parse_mode=ParseMode.HTML, reply_markup=None)
    await callback.message.answer('4/10 - <b><i>Введите общий вес груза(кг)</i></b>:', reply_markup=weight_builder[callback.message.chat.id].as_markup(resize_keyboard=True), parse_mode=ParseMode.HTML)
...
async def order13(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.edit_text('<code>' + callback.message.text + '</code>', parse_mode=ParseMode.HTML, reply_markup=None)
    await callback.message.answer('5/10 - <b><i>Введите общий объем груза(куб. м)</i></b>:', reply_markup=volume_builder[callback.message.chat.id].as_markup(resize_keyboard=True),parse_mode=ParseMode.HTML)
...
async def order17(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.edit_text('<code>' + callback.message.text + '</code>', parse_mode=ParseMode.HTML, reply_markup=None)
    await callback.message.answer('7/10 \- *_Введите ФИО/Название организации отправителя_*', reply_markup=shipper_name_builder[callback.message.chat.id].as_markup(resize_keyboard=True))
...
async def order20(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer('8/10 \- *_Введите номер телефона отправителя_*', reply_markup=shipper_phone_builder[callback.message.chat.id].as_markup(resize_keyboard=True))
...
async def order23(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.edit_text('<code>' + callback.message.text + '</code>', parse_mode=ParseMode.HTML, reply_markup=None)
    await callback.message.answer('9/10 \- *_Введите ФИО/Название организации получателя_*', reply_markup=consignee_name_builder[callback.message.chat.id].as_markup(resize_keyboard=True))
...
async def order_25(callback: types.CallbackQuery, state: FSMContext):
     await callback.message.edit_text('<code>' + callback.message.text + '</code>', parse_mode=ParseMode.HTML, reply_markup=consignee_name_builder[callback.message.chat.id].as_markup(resize_keyboard=True))
...
async def order26(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.edit_text('<code>' + callback.message.text + '</code>', parse_mode=ParseMode.HTML, reply_markup=None)
    await callback.message.answer('10/10 \- *_Введите номер телефона получателя_*:', reply_markup=consignee_phone_builder[callback.message.chat.id].as_markup(resize_keyboard=True))
...
async def order_28(callback: types.CallbackQuery, state: FSMContext):
     await callback.message.edit_text('<code>' + callback.message.text + '</code>', parse_mode=ParseMode.HTML, reply_markup=consignee_phone_builder[callback.message.chat.id].as_markup(resize_keyboard=True))
     await callback.message.answer('10/10 \- _*Введите номер телефона получателя*_:', reply_markup=consignee_phone_builder[callback.message.chat.id].as_markup(resize_keyboard=True))
...
def get_previous(message: types.Message):
    ...
    try:
        for i in range(len(database.select_order(message.chat.id, "departure"))):
            try:
                departure_builder[message.chat.id].add(types.KeyboardButton(
                text = database.select_order(message.chat.id, "departure")[-i]
                    )
                )
                destination_builder[message.chat.id].add(types.KeyboardButton(
                text = database.select_order(message.chat.id, "destination")[-i]
                    )
                )
                pieces_builder[message.chat.id].add(types.KeyboardButton(
                text = database.select_order(message.chat.id, "pieces")[-i]
                    )
                )
                weight_builder[message.chat.id].add(types.KeyboardButton(
                text = database.select_order(message.chat.id, "weight")[-i]
                    )
                )
                volume_builder[message.chat.id].add(types.KeyboardButton(
                text = database.select_order(message.chat.id, "volume")[-i]
                    )
                )
                shipper_name_builder[message.chat.id].add(types.KeyboardButton(
                text = database.select_order(message.chat.id, "shipper_name")[-i]
                    )
                )
                shipper_phone_builder[message.chat.id].add(types.KeyboardButton(
                text = database.select_order(message.chat.id, "shipper_phone")[-i]
                    )
                )
                consignee_name_builder[message.chat.id].add(types.KeyboardButton(
                text = database.select_order(message.chat.id, "consignee_name")[-i]
                    )
                )
                consignee_phone_builder[message.chat.id].add(types.KeyboardButton(
                text = database.select_order(message.chat.id, "consignee_phone")[-i]
                    )
                )
